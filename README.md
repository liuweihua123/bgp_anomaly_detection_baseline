...
| [AP2Vec](AP2Vec/) | BGP 劫持检测相关的 AS/前缀表示学习 | AP2Vec 研究及其 BGP2Vec 基础实现 | [方法与数据说明](AP2Vec/BGP2Vec/README.md)、[`bgp2vec.py`](AP2Vec/BGP2Vec/bgp2vec.py)、[`train_test.py`](AP2Vec/BGP2Vec/train_test.py) | 当前代码以 BGP2Vec、AS 关系分类及预训练模型为主；依赖版本较旧 |
...
├── AP2Vec/           # AP2Vec 相关材料及 BGP2Vec 基础实现
├── BEAM/             # 语义感知的路由异常检测流水线
...
- `AP2Vec/BGP2Vec/requirements.txt` 固定了较旧的完整 Anaconda 环境，其中包含平台相关包；优先在隔离的旧版 Python 环境中按需安装核心依赖。
...
AP2Vec 通过比较新路由表示与历史前缀表示之间的差异来辅助识别 BGP 劫持。当前目录保存了该研究所依赖的 BGP2Vec 基础代码，包括 AS 嵌入模型、AS 关系分类 Notebook、CAIDA AS 关系样例、预处理数据、预训练 Word2Vec 模型和向量查询脚本。此前重复的根目录 `BGP2Vec/` 已移除。

`BGP2VEC` 通过 Python 类调用，没有统一的命令行入口。输入通常为 RouteViews OIX 快照，模型创建与加载逻辑位于 [`bgp2vec.py`](AP2Vec/BGP2Vec/bgp2vec.py)。如需验证仓库中的预训练向量，可进入 `AP2Vec/BGP2Vec/`，确认 Gensim 版本兼容后运行：
...
需要注意：子目录 README 介绍了完整的 AP2Vec 研究思路，但当前可见代码主要覆盖 BGP2Vec 训练、AS 嵌入和 AS 关系分类，未提供完整、独立的 AP2Vec 检测流水线。
...
| AP2Vec | Shapira and Shavitt, *AP2Vec: an Unsupervised Approach for BGP Hijacking Detection*, IEEE TNSM, 2022；目录同时包含其 BGP2Vec 基础实现 |
